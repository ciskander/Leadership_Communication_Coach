      ...
      <div className="flex items-center gap-3">
        {hasActiveExperiment ? (
          <div className="group relative">
            <button disabled className="px-4 py-2 bg-cv-warm-100 text-cv-stone-400 rounded-xl text-xs font-semibold cursor-not-allowed">
              {STRINGS.clientDashboard.accept}
            </button>
            <div className="absolute bottom-full left-0 mb-1.5 hidden group-hover:block z-10">
              <div className="bg-cv-stone-800 text-white text-xs rounded-lg px-3 py-1.5 whitespace-nowrap">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
      <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 space-y-4">
        ...
        <div className="flex gap-2 justify-end pt-1">
          <button onClick={onCancel} disabled={deleting} className="px-4 py-2 text-xs font-semibold text-cv-stone-600 bg-cv-warm-100 rounded-xl hover:bg-cv-warm-200 transition-colors disabled:opacity-50">
            {STRINGS.common.cancel}
          </button>
          <button onClick={onConfirm} disabled={deleting} className="px-4 py-2 text-xs font-semibold text-white bg-cv-red-600 rounded-xl hover:bg-cv-red-700 transition-colors disabled:opacity-50 flex items-center gap-1.5">
            {deleting && (
...
function RecentRunCard({ run, editMode, selected, onToggle }: {
  ...
  if (editMode) {
    return (
      <div
        ...
        className={[
          'flex items-start justify-between px-4 py-3 rounded-xl border gap-4 transition-all select-none',
  ...
  return (
    <Link
      ...
      className="flex items-start justify-between bg-white border border-cv-warm-200 rounded-xl px-4 py-3 hover:border-cv-teal-300 hover:shadow-sm transition-all gap-4"
    >
      {metaContent}{dateLabel}
    </Link>
  );
    ...
    <div className="max-w-3xl mx-auto space-y-6 py-2">
      ...
      <div className="bg-white rounded-2xl border border-cv-warm-200 p-5">
        ...
        {!hasBaseline && !isBuilding && (
          <div className="mt-4 pt-4 border-t border-cv-warm-100 flex items-center justify-between">
            ...
            <Link href="/client/baseline/new" className="text-sm px-4 py-1.5 bg-cv-teal-600 text-white rounded-lg font-medium hover:bg-cv-teal-700 transition-colors whitespace-nowrap">
        ...
        {hasBaseline && !hasExperiment && hasExperimentOptions && (
          <div className="mt-4 pt-4 border-t border-cv-warm-100 flex items-center justify-between">
            ...
            <Link href="/client/experiment" className="text-sm px-4 py-1.5 bg-cv-teal-600 text-white rounded-lg font-medium hover:bg-cv-teal-700 transition-colors whitespace-nowrap">
        ...
        {hasBaseline && !hasExperiment && !hasExperimentOptions && (
          <div className="mt-4 pt-4 border-t border-cv-warm-100 flex items-center justify-between">
            ...
            <Link href="/client/analyze" className="text-sm px-4 py-1.5 bg-cv-teal-600 text-white rounded-lg font-medium hover:bg-cv-teal-700 transition-colors whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-cv-warm-200 p-5 space-y-3">
          ...
          {bpStatus === 'none' && (
            <Link href="/client/baseline/new" className="inline-block text-xs px-3 py-1.5 bg-cv-stone-900 text-white rounded-lg font-medium hover:bg-cv-stone-700 transition-colors">
        ...
        <div className="bg-white rounded-2xl border border-cv-warm-200 p-5 space-y-3">
          ...
          {experiment ? (
            <>
              <div className="bg-cv-teal-50 border border-cv-teal-100 rounded-xl p-3 space-y-1">
                <PatternLabel id={experiment.pattern_id} />
      ...
      <div className="flex gap-3">
        <Link href="/client/analyze" className="flex items-center gap-2 px-5 py-2.5 bg-cv-teal-600 text-white rounded-xl text-sm font-medium hover:bg-cv-teal-700 transition-colors shadow-sm">
          {STRINGS.clientDashboard.analyzeMeetingBtn}
        </Link>
        {bpStatus === 'none' && (
          <Link href="/client/baseline/new" className="flex items-center gap-2 px-5 py-2.5 bg-white border border-cv-warm-300 text-cv-stone-700 rounded-xl text-sm font-medium hover:bg-cv-warm-50 transition-colors">
      ...
      {summary && summary.recent_runs.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              {editMode && selected.size > 0 && (
                <button onClick={() => setShowConfirm(true)} className="text-xs font-semibold text-white bg-cv-red-600 hover:bg-cv-red-700 px-3 py-1.5 rounded-lg transition-colors">
          ...
          {deleteError && (
            <div className="mb-2 text-xs text-cv-red-600 bg-cv-red-50 border border-cv-red-200 rounded-lg px-3 py-2">{deleteError}</div>
          )}
          {editMode && (
